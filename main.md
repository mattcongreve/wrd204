...
This guide is primarily aimed towards Windows users. While some steps may be similar betwen others may be Windows specific.
...
Once you have python installed, it's recommended to setup a "Virtual environment" to contain all of the packages you intend to use for this project.
...
Afterwords you'll need to use the package manager `pip` in order to install 2 packages you'll need for this tutorial:
...
To get started with using the falcon API framework, you need to build an application that can be hosted on a web server. The basic, bare-bones structure of a Falcon application is as follows:
...
And that's it! To launch your application, execute `waitress-serve` against the application name and the app instance variable name. In this case, I named my file `example.py`.
...
However, when you run this, you should recieve an error.
...
Routes are added to falcon applications via the function `add_route` against the falcon API instance. These routes are defined via a string indicating the path to be reached after the url, and then the definition of a resource. For example:

```python
add_route('/example', example_class)
```

Resources are python classes that are defined to help indicate what a web request does when reaching that specific route. In the case of the url you would be testing on your local desktop, this would be `http://localhost:8000/example`
...
Each of these functions will contain the logic behind what you intend to do behind each type of API request. You may notice that there are `self`, `req` and `resp` arguments being given to each of these classes. `self` is a just a widely accepted naming sytnax to use when accessing the initialized properties of the clasee, defined by `__init__`. `req` and `resp` are specific arguments used to access certain properties of your incoming request. It's imporant to understand that these different responders should reflect what is being done on the backend of your API. 

### The Request and Response classes

Both the request and response classes are what help you consume the incoming data on a request, transform that data into something actionable, and then modify the response to either indicate success or deliver data.

The GET method of an RESTful web API is commonly used to specifically retreive data, and have it delivered to the response. In a basic implementation, that would look like this:
...
There are multiple attributes that can be set in the response class to define how you want to respond to specific request. The most important ones to note are `body`, `media`, and `status`. Above, we used `body`. This defines the raw content that you are delivering in your response. In API implementations, you're usually returning your data back in a serilizable method, whether that be with JSON, XML, YAML, etc. Falcon has a built in processor for this data that is defined by the `media` attribute. By default, this media is processed and converted from a python object into JSON representation. 
...
class ExampleClass(object):
    ...
    def on_get(self, req, resp):
        try:
            with open(self.full_path, 'r') as fd: # retrieveing content
                content = fd.read(fd)
            resp.media = {'ok': True, 'content': content} #presenting content in similar JSON structure as POST
        except Exception as ex:
            resp.media = {'ok': False, 'content': None, 'exception': repr(ex)} # Retruning an error response and proper code like up above
...
Our API returned a success, therefore, we have recieved confirmation that the data we have written is present. Now let's get that data with a GET request.
